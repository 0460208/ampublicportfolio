...
{
   $namesbox.='<option value="' . $row['Account_id'] . '">' . $row['Account_lastname'] . ', ' .$row['Account_firstname'] . ' ('.$row['Account_id'] .') ' . '</option>';
}
$namesbox.='</select>';
...
$empposbox.='<optiion value="' . $row1['GL_data']. '">'.$row1['GL_data'].'</option>';
}
while($row = mysqli_fetch_array($query_run1,MYSQLI_BOTH)) 
{
...
<div class="label-wrapper">
Employee Number: 
<input type="text" placeholder=<? echo $employeeNumber ?> name="employeeNumber" size="38" readonly/>
</div>
