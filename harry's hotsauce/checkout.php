...
<br /><br /><br /><br />
<div style="width:310px; height:auto; float:left;">Payment Method: 
...
<div style="width:300px; height:auto; float:left;">Exp Date(mm/yy):
...
//if the submit checkout button was clicked all HTML form names change into php variables
  ...
  //order number from build_order function
  $Order_Number = $_SESSION['OrderNumber'];
  
  //update the account with the current information
$query1 = "UPDATE `Accounts` SET `Account_lastname` =
'".$lName."', `Account_firstname` = '".$fName."', `Account_address1` =
'".$address1."', `Account_city` = '".$city1."', `Account_state_region` =
'".$state1."', `Account_postal_code` = '".$zip1."', `Account_email_address` =
'".$emailA."', `Account_last_update_date` = now(), `Account_last_update_by` = 'Check Out' WHERE `Account_id` = '".$accountID."'";
...
//Insert payment into payment method table then retrieve the PM_id number

$query2 = "INSERT INTO `Payment_method` (`PM_type`, `PM_number`, `PM_card_security_code`, `PM_exp_date`, `PM_account_id`, `PM_created_date`,
`PM_created_by`, `PM_last_updated_date`, `PM_last_updated_by`) VALUES 
('$payment', '$cCN', '$cVV', STR_TO_DATE('$expDate','%m/%y'), '$accountID', now(), 'Check Out', now(), 'Check Out')";
...
if (!$result3 = mysqli_query($link, $query3))
  ...
  //update order headers with status 2 which is processed 
  $query4 = "UPDATE `Order_headers` SET `Order_header_status_id` = '".$status."', `Order_header_pm_id` = '".$PM_id."', 
  `Order_header_last_updated_date` = now(), `Order_header_last_updated_by` = 'Check Out' WHERE `Order_header_account_id` = $accountID AND 
 ...
 echo "<script type='text/javascript'> document.location = 'receipt.php';
 </script> ";
 }
 ?>
 </div>
