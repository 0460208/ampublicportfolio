...
$query = "SELECT * FROM Products, Images, Inventory WHERE
`Products_image_id` = `Images_id` AND `Inventory_product_id` =
 `Products_id`";
if (!$result = mysqli_query($link, $query))
 {
    $result = 0;
  ...
  while($row = mysqli_fetch_array($result, MYSQLI_BOTH))
{
  ...
  $query16a = "SELECT SUM(SC_order_quantity) FROM Shopping_cart WHERE
  SC_inventory_id = $inv_id";
  ...
  if($availqty > 0)
  {
    // If units in stock are greater than display the avaiable amount
    print 'Units Available - '. $availqty;
  }
    else
  {
    // if they are less than 0 display
    print 'Out of Stock';
  }
  print '</div>'; //end of inventory
  print '<div class="price">';
  print '$'.number_format($row['Products_unit_price'],2);
  print '<input class="quantity" type="text" name="quantity1"
  placeholder="0"/>';
  print '</div>'; //end of price
  if($availqty <= 0)
  {
    // if the units in stock are less than or equal to 0 display unavailable
...
} // end of if else (!$result = mysqli_query($link, $query16a))
print '</div>'; //end of item-wrapper
print '</form>';
} //end of while loop
mysqli_free_result($result); // clears result set.
mysqli_free_result($result1); // clears result1 set.
...
$query5 = "SELECT `Products_unit_price` FROM `Products`, Inventory WHERE
`Inventory_id` = $id and Inventory_product_id = Products_id";
...
$query16 = "SELECT * FROM `Shopping_cart` WHERE `SC_inventory_id` = $id and
SC_account_id = $accountID";
if (!$result = mysqli_query($link, $query16))
{
   $result = 0;
   echo ('Error executing query4: ' . mysqli_errno($link)." -
  ".mysqli_error($link)."<BR>");
}
else
{
...
if($sCInvId != $id)
{
  ...
  $query8 = "INSERT INTO Shopping_cart (`SC_account_id`, `SC_inventory_id`,
  `SC_order_quantity`, `SC_unit_price`, `SC_created_date`, `SC_created_by`,
  `SC_last_updated_date`,  `SC_last_updated_by`) VALUES ('$accountID', '$id',
  '$quantity', '$unitPrice', '$dateTime', 'addToCartButton', '$dateTime',
    'addToCartButton' )";

  if (!$result = mysqli_query($link, $query8))
  {
     $result = 0;
     echo ('Error executing query5: ' . mysqli_errno($link)." -
    ".mysqli_error($link)."<BR>");
    }
  }
  else {
    // if it does equal the ID select all from the shopping cart
    $query19 = "SELECT * FROM `Shopping_cart` WHERE `SC_inventory_id` = $id and
    SC_account_id = $accountID ";
    ...
    // shopping cart order quantity gets added to a variable
    ...
    $sql18 = "UPDATE `Shopping_cart` SET `SC_order_quantity` =
    '".$scSameItemDiffQuan."' WHERE `SC_inventory_id` = $id and SC_account_id =
    $accountID";
    if (!$result = mysqli_query($link, $sql18))
    {
     $result = 0;
     echo ('Error executing query6: ' . mysqli_errno($link)." -
    ".mysqli_error($link)."<BR>");
    }
  }
  }
...
} // end of if(isset($_POSTrsubmitItem') and $_POST['quantity1'] > 0)
